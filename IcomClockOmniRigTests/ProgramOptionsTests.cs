...
namespace IcomClockOmniRig.Tests {
    ...
    public class ProgramOptionsTests {
        ...
        readonly string help_ending =
            ...
            "\t\t\tValid models: IC-705, IC-7100, IC-7300, IC-7300MK2, IC-7600, IC-7610, IC-7700, IC-7850, IC-7851, IC-9700, IC-R8600, IC-R9500\n\n" +
        ...
        public void Args_Help_Quiet() {
            ...
            using (StringWriter sw = new StringWriter()) {
                Console.SetOut(sw);
                try {
                    ProgramOptions programOptions = new ProgramOptions(new string[2] { "-h", "-q" });
                } catch (ExitException e) {
                    // Help throws an ExitCode.SUCCESS to terminate the program nicely.
                    Assert.AreEqual(ExitCode.SUCCESS, e.ExitCode);
                    Assert.AreEqual("", e.Message);
